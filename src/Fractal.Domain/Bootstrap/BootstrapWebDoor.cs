...
namespace Fractal.Domain.Bootstrap
{
...
//              FractalDb.AddDomainConceptField( "validation", "funcstr");
            ...
            FractalDb.CreateDomainConcept("PageController");
                ...
                FractalDb.AddDomainConceptField("PageController", "JSON");
...
//            FractalDb.AddDomainConceptField("Snippetf", FF.FFUNC);
            ...
            DomainConceptInstance indexPageController = FractalDb.CreateDomainConceptInstance("PageController", "/", "False", "HandleIndex", "public FModel HandleIndex(Request request, DomainConceptInstance thisPc) { FModel fmodel = new FModel(); fmodel.Request = request;fmodel.ThisDci = thisPc; return fmodel; }");
            FractalDb.Connect(emidiumSite, indexPageController, "SiteToPageControllers");
            FractalDb.Connect(indexPageController, FractalDb.CreateDomainConceptInstance("WebPage", "/", GetFileAsResource("index.cshtml")), "PageControllerToPage");
        }

        private static string GetFileAsResource(string fileName)
        {
            Assembly executingAssembly = Assembly.GetExecutingAssembly();
            StreamReader streamReader = new StreamReader(executingAssembly.GetManifestResourceStream("Fractal.Domain.Bootstrap." + fileName));
